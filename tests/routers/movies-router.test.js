...
describe('Movies Controller', () => {
  describe('GET /movies', function () {
    context('without query params', () => {
      it('should have status code 200', async function () {
        await supertest(app).get('/movies').send({}).expect(200)
      })
    })
      ...
      context('when limit=10 and offset=10', () => {
        it('should return 10 items', async () => {
          const response = await supertest(app)
            .get('/movies?limit=10&offset=10')
            .send({})
            .expect(200)

          expect(response.body).to.exist
          expect(response.body.total).to.eq(10)
          expect(response.body.movies.length).to.eq(10)
        })
      })
    })
  })
  ...
  describe('GET /movies/:id', function () {
    context('when correct id is given', () => {
      it('should have status code 200 with the correct movie object', async function () {
        const existingMovieId = INITIAL_MOVIES.movies[INITIAL_MOVIES.movies.length - 1]
        const response = await supertest(app).get(`/movies/${existingMovieId.id}`).expect(200)

        expect(response.body).to.exist
        expect(response.body.id).to.eq(existingMovieId.id)
        expect(response.body.title).to.eq(existingMovieId.title)
        expect(response.body.img).to.eq(existingMovieId.img)
        expect(response.body.synopsis).to.eq(existingMovieId.synopsis)
        expect(response.body.rating).to.eq(existingMovieId.rating)
        expect(response.body.year).to.eq(existingMovieId.year)
      })
    })

    context('when id of movie doesnt exist', () => {
      it('should return status code 404', async () => {
        await supertest(app).get('/movies/555').send({}).expect(404)
      })
    })
  })
  ...
  describe('POST /movies', function () {
    it('should create a new movie object', async function () {
      ...
      const response = await supertest(app).post(`/movies`).send(newMovieJson).expect(201)
      expect(response.body).to.exist
      expect(response.body.id).to.be.greaterThan(lastId)
      expect(response.body.title).to.eq(newMovieJson.title)
      expect(response.body.img).to.eq(newMovieJson.img)
      expect(response.body.synopsis).to.eq(newMovieJson.synopsis)
      expect(response.body.rating).to.eq(newMovieJson.rating)
      expect(response.body.year).to.eq(newMovieJson.year)
    })
  })
